...
def build_dataloader(df, train_val_flag=True, fold=None, data_transforms=None):
    if train_val_flag:
        ...
        test_loader = DataLoader(test_dataset, batch_size=CFG.train_bs, num_workers=0, shuffle=False, pin_memory=True, drop_last=False) # False False
        return test_loader
...
def train_one_epoch(model, train_loader, optimizer, losses_dict, CFG):
    ...
    for _, (images, gt) in pbar:
        ...
        # 反向传播 用scaler，scale loss(FP16)，backward得到scaled的梯度(FP16)
...
if __name__ == '__main__':
    ...
    class CFG:
        ...
        ckpt_fold = "output"
        ckpt_name = "efficientnetb4_img224224_bs4"  # for submit.
    ...
    if train_val_flag:
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(df)):
            df.loc[val_idx, 'fold'] = fold  # 在df的fold的列中，将val_idx置为fold
    ...
    if test_flag:
        ...
        data_transforms = build_transforms(CFG)  
        test_loader = build_dataloader(test_df, fold , data_transforms)  # dataset & dtaloader
        
        ###############################################################
        ##### >>>>>>> step3: test <<<<<<
        ###############################################################
        ckpt_paths = ["dummy_test"] # please use your ckpt path
        test_df = test_one_epoch(test_df, ckpt_paths, test_loader, CFG)
    ...
    if test_flag:
        ...
        ckpt_paths = ['./output/efficientnetb4_img224224_bs4/best_fold3_epoch6.pth']
        test_df = test_one_epoch(test_df, ckpt_paths, test_loader, CFG)
        submit_df = test_df.loc[:, ['img_name', 'pred_prob']]
        submit_df.to_csv("submit_dummy.csv", header=False, index=False, sep=' ') 
